...
class MultivariableFunction:
    ...
    def tangent_hyperplane(self, point: list)-> sp.Expr:
        '''
    ...
    def directional_diff(self, direction_vect: list[float|int])-> Callable:
    ...
    def multivar_integral(self, bounds: list[tuple[float, float]])-> int|float:
        '''
    ...
    #HELPER FUNCTION
    def _find_local_extrema(maxima_or_minima: str, independent_vars: list, expr: sp.Expr, initial_point: list, step_size: float | int, norm_tolerance: float, hypershperical_constraint: int, verbose: bool = False)-> np.ndarray:
    ...
    def _parallel_find_local_extrema(params)-> np.ndarray:
